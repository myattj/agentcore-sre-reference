...
def _bindings_block(
    ...
    guidance = (
        "**Picking the right one:** match the user's intent. "
        "If they name a repo — by full slug, short name, or alias — "
        "use that repo. If their question clearly refers to a "
        "different connected repo than the default, switch to it. "
        "Only stop to ask when the message is genuinely ambiguous "
        "and you can't tell from context which repo is meant.\n\n"
        ...
        "**When you do need to ask:** call ``ask_codebase_choice`` "
        "with the candidate slugs to post Slack buttons — that's a "
        "nicer UX than free-text back-and-forth. Keep the clarifying "
        "message short."
    )
