...
Agent is a self-hosted open-source project. It is not a managed security product,
has not received an independent security audit, and carries no response-time or
remediation SLA. Each operator owns the security of their deployment.

## Supported versions

| Version | Security support |
|---|---|
| Current <code>main</code> branch | Best-effort fixes and review |
| Historical tags, forks, and deployed copies | Operator maintained |

Pin releases and review changes before upgrading. Dependencies, cloud APIs,
model behavior, and platform defaults can change independently of this project.
...
A report may not receive a timely response. If you operate a deployment, you
remain responsible for containment, patching, credential rotation, and incident
response.
...
- **PR sandbox:** disabled by default. The experimental Fargate worker executes model-generated shell commands in the same task that receives the model key, callback secret, and GitHub installation credential. Containerization does not protect those in-process credentials from the model-driven shell. Use only disposable credentials and accounts; a production design needs a trusted credential broker, short-lived repository-scoped tokens, stronger isolation, restricted egress, output scanning, spend caps, and human approval.
...
- **Onboarding identity:** the built-in session-token flow is not a finished multi-user RBAC system. The bridge sets a host-scoped HttpOnly cookie and redirects to a clean URL, so production deployments must route the bridge callback and onboarding UI through one HTTPS public origin. Never move the session bearer into a URL.
